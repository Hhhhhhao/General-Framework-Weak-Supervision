...
class NormScaleClassifier(nn.Module):
    def __init__(self, in_dim, out_dim, scale=10.0, *args, **kwargs):
...
class WideResNet(nn.Module):
    def __init__(self, first_stride, num_classes, depth=28, widen_factor=2, drop_rate=0.0, **kwargs):
        ...
        self.classifier = nn.Linear(channels[3], num_classes) # nn.Linear(channels[3], num_classes)
        # self.classifier = NormScaleClassifier(channels[3], num_classes) # nn.Linear(channels[3], num_classes)
...
def wrn_34_10(pretrained=False, pretrained_path=None, **kwargs):
    model = WideResNet(first_stride=1, depth=34, widen_factor=10, **kwargs)
    if pretrained:
        model = load_checkpoint(model, pretrained_path)
    return model