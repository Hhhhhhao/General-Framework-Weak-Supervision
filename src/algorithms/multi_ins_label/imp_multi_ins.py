...
from src.core.nfa import create_multi_ins_graph


from src.datasets import get_multi_ins_bags_labels, get_data, get_dataloader, bag_collate_fn, ImgBaseDataset, ImageTwoViewBagDataset


class ImpreciseMultipleInstanceLearning(AlgorithmBase):
    def __init__(self, args, tb_log=None, logger=None, **kwargs):
        ...
        self.p_model = self.p_model.cuda(self.gpu)
        self.m = 0.999
    ...
    def init(self, args):        
        ...
        self.balanced_bags = args.balanced_bags
    ...
    def set_dataset(self):
        ...
        # make partial labels
        train_bag_data, train_bag_targets = get_multi_ins_bags_labels(train_data, train_targets, self.num_classes, 
                                                                      target_classes=self.target_classes, 
                                                                      class_map=self.class_map,
                                                                      mean_bag_len=self.mean_bag_len, std_bag_len=self.std_bag_len, num_bags=self.num_bags_train, 
                                                                      balanced_bags=self.balanced_bags)
    ...
    def train_step(self, x_bag_w, x_bag_s, x_bag_len, y_bag):      
        ...
        for target_class in self.target_classes:
            ...
            # pad probs_x_w
            pad_binary_probs_x_w = pad_sequence(binary_probs_x_w.split(x_bag_len.tolist()), batch_first=True)
            # forward-backward
            pad_binary_pseudo_y_ins, binary_probs_y_bag = create_multi_ins_graph(torch.log(pad_binary_probs_x_w), x_bag_len, binary_y_bag)
            # unpad pseudo_y_ins
            binary_pseudo_y_ins = unpad_sequence(pad_binary_pseudo_y_ins, x_bag_len, batch_first=True)
            binary_pseudo_y_ins = torch.cat(binary_pseudo_y_ins, dim=0)
            ...
            # compute unsup loss
            masks = binary_pseudo_y_ins[:, 0] == 1.0
            if masks.sum() == len(masks):
                unsup_loss += 0
            else:
                unsup_loss += (torch.sum(-binary_pseudo_y_ins.detach() * torch.log(binary_probs_x_s), dim=1) * masks).sum() /  (len(masks) - masks.sum())
            
            # comput sup loss
            sup_loss += self.bce_loss(binary_probs_y_bag, binary_y_bag.to(torch.float32), reduction='mean')
        
        unsup_loss /= len(self.target_classes)
        sup_loss /= len(self.target_classes)
        total_loss = unsup_loss + sup_loss
        
        
        out_dict = self.process_out_dict(loss=total_loss)
        log_dict = self.process_log_dict(loss=total_loss.item(),  unsup_loss=unsup_loss.item() if not isinstance(unsup_loss, float) else unsup_loss, sup_loss=sup_loss.item())
    ...
    def get_argument():
        return [
            Argument('--target_classes', int, 9, 'target class for multiple instance learning', nargs='+'),
