...
def dp_count_proportion(log_probs, lengths, targets):
    ...
    for i in range(batch_size):
        ...
        count_prob = normalized_alpha[targets[i]].view(1, -1)
...
class CountLossProportionLabelLearning(ImpreciseProportionLabelLearning):
    ...
    def train_step(self, x_bag_w, x_bag_len, y_bag):      
        ...
        for cls_idx in self.cls_idx_list:
            binary_y_bag = y_bag[:, cls_idx].to(torch.long)
            ...
            # dynamic programming
            sup_preds = dp_count_proportion(torch.log(pad_binary_probs_x), x_bag_len, binary_y_bag)
        
            loss +=  -torch.log(sup_preds + 1e-12).sum(dim=-1).mean()
